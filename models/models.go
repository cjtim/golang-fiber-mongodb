...
func (s *Models) FindAll(collectionName string, results interface{}, filter bson.M) {
	var tmp []bson.M
...
func (s *Models) FindOne(collectionName string, results interface{}, filter bson.M) {
	var tmp bson.M
	collection := s.Client.Database(os.Getenv("MONGO_DB")).Collection(collectionName)
